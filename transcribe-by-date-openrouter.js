...
if (!dateArg) {
  ...
  console.error(
    "Usage: node transcribe-by-date-openrouter-gemini.js 2023-01-15"
  );
  process.exit(1);
}
...
if (!dateRegex.test(dateArg)) {
  ...
  console.error(
    "Example: node transcribe-by-date-openrouter-gemini.js 2023-01-15"
  );
  process.exit(1);
}
...
async function main() {
  ...
  if (!fs.existsSync(imagePath)) {
    ...
    console.error(
      `💡 Make sure the comic exists in the correct directory structure.`
    );
    console.error(
      `   Expected path: source-data/dilbert-comics/${year}/${dateArg}.gif`
    );
    process.exit(1);
  }
  ...
  if (fs.existsSync(transcriptPath)) {
    ...
    try {
      ...
      console.log(
        `❌ Error reading existing transcript, will recreate: ${error.message}`
      );
    }
  }

  console.log(
    `🤖 Using Google Gemini via OpenRouter (${MODEL_NAME}) for transcription...`
  );
