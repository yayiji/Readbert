#!/usr/bin/env node
/**
 * Dilbert Comics Bulk Transcription Script (OpenRouter Gemini API)
 * 
 * This script transcribes multiple Dilbert comics using OpenRouter's Gemini API.
 * Based on the successful single-date transcription approach.
 * 
 * USAGE:
 *   node transcribe-bulk-openrouter-gemini.js [YYYY] [start-date] [end-date]
 * 
 * EXAMPLES:
 *   node transcribe-bulk-openrouter-gemini.js 2023                    # Transcribe entire year 2023
 *   node transcribe-bulk-openrouter-gemini.js 2023 2023-01-01 2023-01-31  # Transcribe January 2023
 *   node transcribe-bulk-openrouter-gemini.js                        # Interactive mode - prompts for input
 * 
 * REQUIREMENTS:
 *   1. Create .env file with: OPENROUTER_API_KEY=your_api_key_here
 *   2. Comic images must exist at: source-data/dilbert-comics/YYYY/YYYY-MM-DD.gif
 * 
 * OUTPUT:
 *   - Saves transcripts to: static/dilbert-transcripts/YYYY/YYYY-MM-DD.json
 *   - Progress tracking with detailed logging
 *   - Automatic retry on failures
 *   - Skips already transcribed comics
 * 
 * FEATURES:
 *   - Uses proven OpenRouter Gemini 2.5 Flash Lite model
...
if (args.length === 0) {
    ...
    console.log('  node transcribe-bulk-openrouter-gemini.js 2023');
    console.log('  node transcribe-bulk-openrouter-gemini.js 2023 2023-01-01 2023-01-31');
    process.exit(0);
    ...
    console.error('❌ Invalid arguments. Use: node transcribe-bulk-openrouter-gemini.js [YYYY] [start-date] [end-date]');
    process.exit(1);
}